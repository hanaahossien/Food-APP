...
import { axiosInstance, userRecipe } from "../../../../services/apiUrls/apiUrl";
export default function RecipeList() {
  ...
  let addToFav = async (recipeId) => {
    try {
      const res = await axiosInstance.post(userRecipe.AddToFav, {
        recipeId:`${recipeId}`
      });
      getRecipesList();
      toast.success("added  to  favorite Recipe");

      navigat("/dashboard/fav");
    } catch (error) {
      toast.error(error);
    }
  };
                  ...
                  {RecipesList.map((recipe) => (
                    <tr key={recipe.id}>
                      ...
                      {loginData.userGroup == "SystemUser" ? (
                        <td className="py-4">
                          <i role="button"
                            class="fa-solid fa-heart-circle-plus fs-3 text-danger"
